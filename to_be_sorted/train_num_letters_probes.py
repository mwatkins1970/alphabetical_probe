def train_num_letters_probes(embeddings, token_length_dict, all_rom_token_indices): 
  ...
  num_letters_probe_weights_tensor = torch.zeros(26, 4096)
  ...
  for num_letters in range(1,13):    # there are not enough tokens with 13 or 14 distinct letters to work with

    # construct tensors of embeddings and labels for training and validation
    all_embeddings, all_labels = get_training_data_num_letters(num_letters, num_samples, embeddings, token_length_dict, all_rom_token_indices)
    ...
    for epoch in range(num_epochs):
        ...
        print(f"{num_letters} total letters: epoch {epoch + 1}/{num_epochs}, Loss: {total_loss / len(train_loader)}")

    # STORE THE PROBE WEIGHTS (or "direction" in embedding space associated with this probe)
    distinct_letters_probe_weights_tensor[num_letters] = model.fc.weight.data.clone().detach()
  ...
  print("\nSummary:")
  print("Total no. letters | Best Train Loss | Validation Loss | Validation Accuracy")
  print("-" * 75)
  for num_letters, metrics in results.items():
      print(f"{num_letters}              | {metrics['best_train_loss']:.4f}           | {metrics['validation_loss']:.4f}        | {metrics['validation_accuracy']:.4f}")

  print("-" * 75)

  print(num_letters_probe_weights_tensor)

  return num_letters_probe_weights_tensor