# This builds a dataset for training a linear probe to classify tokens according to presence of a given adjacent letter triple


def get_training_data_anywhere_triple(target_triple, num_samples, embeddings, all_rom_token_gt2_indices):

	# Fetch indices for tokens that contain the specified adjacent-letter-triple
	positive_indices = [index for index in all_rom_token_gt2_indices if target_triple.lower() in token_strings[index].lstrip().lower()]

	# Fetch indices for tokens that do contain it
