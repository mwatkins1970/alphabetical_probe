# This builds a dataset for training a linear probe to classify tokens according to whether they start with a given adjacent letter pair


def get_training_data_starting_double(target_double, num_samples, embeddings, all_rom_token_gt2_indices):

	# Fetch indices for tokens that begin with the specified adjacent-letter-pair
	positive_indices = [index for index in all_rom_token_gt2_indices if token_strings[index].lstrip().lower()[0:2] == target_double.lower()]
