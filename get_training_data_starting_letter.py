...
def get_training_data_starting_letter(target_letter, num_samples, embeddings, all_rom_token_gt2_indices):

	# Fetch indices for tokens that begin with the specified letter
	positive_indices = [index for index in all_rom_token_gt2_indices if token_strings[index].lstrip().lower()[0] == target_letter.lower().strip()]
